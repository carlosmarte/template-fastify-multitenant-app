...
const contexts = {
  ...
  config: {
    ...
    plugins: {
      coreOrder: ["auth", "cookie", "exception", "logger", "request", "static"],
    },
  },
};
      ...
      get: function (_, name) {
        return async function (app, options = {}) {
          ...
          if (typeof contexts.plugins[name] === "function") {
            await app.register(contexts.plugins[name], options);
...
contexts.capabilities = {
  ...
  loadPlugin: async (app, pluginPath, options = {}) => {
    try {
      ...
      app.log.info(`Loaded plugin from ${pluginPath}`);
      return true;
    } catch (err) {
  ...
  loadSchema: async (app, schemaPath) => {
    try {
      ...
      app.log.info(`Found ${schemaFiles.length} schema files in ${schemaPath}`);
  ...
  loadTenant: async (app, tenantId) => {
    try {
      ...
      if (existsSync(servicesPath)) {
        ...
        app.log.info(
          `Loaded ${Object.keys(services).length} services for tenant ${customTenantId}`
        );
      }
...
export async function start(options = {}) {
  ...
  try {
    ...
    if (existsSync(pluginsDir)) {
      ...
      for (const pluginName of pluginDirs) {
        if (pluginName.startsWith(".") || pluginOrder.includes(pluginName))
          continue;
